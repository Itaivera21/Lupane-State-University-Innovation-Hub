...
def request_supervisor(project_id):
    try:
        ...
        # FIXED: Do NOT assign supervisor_id immediately
        # Only set status to pending_supervision
        # The supervisor will approve/assign themselves later
        project.status = 'pending_supervision'
        project.supervision_requested_at = datetime.utcnow()
        # Store the requested supervisor ID in a temporary field or just show in pending list
        # For now, we'll store it in supervisor_id but mark as unapproved
        # Actually, let's NOT store supervisor_id until approval
        # The supervisor will see the pending request and can approve it
        db.session.commit()
        
        # Optional: Store the request in a separate table or just rely on status
        # For now, the supervisor will see all pending_supervision projects assigned to them
        # We need to link the request to the supervisor
        # Let's store the requested supervisor in a separate column or just set supervisor_id but mark as pending
        # Revert: Store supervisor_id but keep status as pending_supervision
